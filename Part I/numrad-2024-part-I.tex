...
\begin{center}\Large
Take away \#1:\\
\textit{What are the basic structures of PDE-constrained optimization under uncertainty? }
\end{center}
}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{exampleblock}{}
\centering
We don't wish to \alert{quantify uncertainty}, but to \alert{make decisions} to \alert{minimize uncertainty}.
\end{exampleblock}
}
...
\begin{exampleblock}{}
\centering
This leads to the focus of our course:\\ \textbf{PDE-Constrained Optimization under Uncertainty.}
\end{exampleblock}
}
\end{frame}
...
\visible<3->{
\item Stochastic state constraints are very challenging: See C.\ Geiersbach's presentation!
}
\end{itemize}
\end{exampleblock}
...
\begin{figure}[b]
    \centering
    \includegraphics[width=0.3\linewidth]{figures/oos_my_g1000_M2000.pdf}
    % \caption{A}
    %\label{fig:enter-label}
\end{figure}
     % \includegraphics[width=\linewidth]{figures/oos_my_g1000_M2000.pdf}
...
\begin{center}\Large
Take away \#2:\\
\textit{What do we \alert{actually} control in OUU?}
\end{center}
\end{frame}
...
\begin{exampleblock}{}
    \centering
    The best we can do is \alert{shape the distribution} of the objective.
\end{exampleblock}
}
\end{frame}
...
\begin{center}\Large
Take away \#3:\\
\textit{What do we mean by \alert{risk-averse}? }
\end{center}
}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{center}\Large
Section 2\\
Basic Theory: How to handle PDE-constraints
\end{center}
\end{frame}
...
\only<4>{
\begin{exampleblock}{}
\centering Without single-valued $S(z)$, theory and algorithmic approaches are more difficult.\\ Leads to 
\textbf{full space approach}.
\end{exampleblock}
}
\end{frame}
...
\begin{center}\Large
Take away \#4:\\
\textit{Don't forget to use \alert{Riesz mappings} for your discrete gradients? }
\end{center}
}
\end{frame}
...
\item $f:X \to \mathbb R$ is typically continuous, \textbf{differentiable}.
...
\begin{itemize}
\item \visible<2->{
\textbf{``PDE''-part}: $X$ replaced by $\{X_h\}_{h > 0}$
%, $X_h$  finite dim.\ with elements $u_{h}(x) = \sum_{i=1}^{N} u^h_i \varphi^h_i(x)$.
}
\item \visible<3->{
\textbf{``PDE''-part}: $F(x)$ requires the solution of a PDE with input $x$.
}
\item \visible<4->{
\textbf{``Random"-part}: cannot evaluate $F(x)$ $x \in X$ or even $F_h(x_h)$ $x_h \in X_h$.
}
...
\]
\item $\mathfrak{R}_{\rm Riesz}$ requires the solution of Poisson problem in $H^1_0(D)$.
}
\end{itemize}
\end{example}
\end{frame}
...
\begin{itemize}
\item \textbf{SA}: \textbf{S}tochastic \textbf{A}pproximation.
\visible<2->{
...
\begin{frame}\frametitle{Disadvantages \alert{(Opportunities?)}}
\begin{exampleblock}{}
\begin{itemize}
\visible<1->{
