...
$z$ $\sim$ material density, 
$a \equiv 0$, $b \equiv 1$, 
$\Phi(z) = \int_{D} z \ \mathrm{d}x$.
...
\end{exampleblock}
\visible<2->{
\begin{example}[Stochastic State Constraints]
\begin{itemize}
\item Option A: $p \in (0,1)$ 
\[
\varphi(u) := \mathbb P( \left\{\omega \in \Omega \left| u(x,\omega) \le T(x) \text{ a.e. x} \in D\right.\right\}) \ge p.
\] 
\item Option B: $p = 1$
\[
\mathbb P( \left\{\omega \in \Omega \left| u(x,\omega) \le T(x) \text{ a.e. x} \in D\right.\right\}) = 1.
\] 
\end{itemize}
\begin{itemize}
    \item Option A is mathematically hard: continuity, differentiability of $\varphi$ nontrivial
    \item Option B is closer to deterministic case. Probably too restrictive.
    % \item Plenty of ongoing work, see reference list.
\end{itemize}
\end{example}
}
\end{frame}

\begin{frame}\frametitle{What exactly would I like to optimize?}
...
\end{itemize}
\end{block}
\begin{example}[Linear Elliptic Boundary Value Problem (Neumann)]
...
\end{example}\pause
\begin{example}[Nonlinear Boundary Value Problem (Allen-Cahn)]
...
%\begin{frame}\frametitle{The Lax-Milgram Lemma}
%\begin{lemma}[Lax-Milgram]
%Let $V$ be a Hilbert space and $a : V \times V \to \mathbb R$ a bounded, coercive bilinear form. I.e. there exist constants $C, c > 0$ for all $u,v \in V$ such that
%\begin{itemize}
%\item $|a(u,v)| \le C \| u \|_{V} \| v \|_{V}$,
%\item $a(u,v) \ge c\| u \|^2_{V}$.
%\end{itemize}
%Then for any $f \in V^*$, the topological dual of $V$, there is a unique solution $u \in V$ to the variational equation:
%\[
%a(u,v) = L(v),\quad \forall v \in V
%\]
%such that 
%$
%\| u \|_{V} \le \frac{1}{c} \| f \|_{V^*}.
%$
%\end{lemma}
%\begin{block}{Remarks}
%\begin{itemize}
%\item Both the existence, uniqueness, and bound provided by the Lax-Milgram-Lemma will be crucial for our model PDE with random inputs!
%\item We typically do not identify a Hilbert space with its dual in optimal control and PDE-constrained optimization, as the Riesz map is generally non-trivial. For $L^2(D)$, however, it is trivial. Therefore, we always take $L^2(D) = (L^2(D))^*$
%\end{itemize}
%\end{block}
%\end{frame}
...
%We define the following parametric variational problem:\\
...
%\begin{itemize}
%\item What is an appropriate way of viewing \eqref{eq:lin_ellip_stoch}?
%\item What would be the right function space setting?
%\item What kind of regularity do we require of $A, f$?
%\end{itemize}
%\end{block}
\end{frame}
...
%\end{block}
%\end{frame}
%\begin{frame}\frametitle{Existence and Regularity of Solutions}
%\begin{block}{}
%\begin{enumerate}
%\item
%\end{enumerate}
...
% \begin{frame}\frametitle{Challenges: Nonlinear and Evolution Equations}
% \begin{block}{Remarks}
% \begin{itemize}
% \item The classical theory yields not only existence and uniqueness of a solution, but also measurability, integrability, etc. for the linear case.
% \item The situation is more challenging for nonlinear elliptic PDE, e.g., 
% \begin{equation*}
% \aligned
% -\xi(\omega)\Delta u + N(u,\omega)        &= f, \text{ in } D,\text{ a.e. } \omega \in \Omega,\\
% \xi(\omega) \partial_{\bf n} u &= z,\text{ on } \Gamma,\text{ a.e. } \omega \in \Omega,
% \endaligned
% \end{equation*}
% as there is no general means of obtaining measurability and integrability.
% \item Problems involving time-dependence, e.g., 
% \begin{equation*}
% \aligned
% \frac{\partial u}{\partial t} -\xi_t(\omega)\Delta u  &= f, \text{ in } D \times (0,T),\text{ a.e. } \omega \in \Omega,\\
% \xi_t(\omega) \partial_{\bf n} u &= z,\text{ on } \Gamma \times (0,T),\text{ a.e. } \omega \in \Omega,\\
% u(0) &= u_0
% \endaligned
% \end{equation*}
% present an even greater challenge ($\xi_t$ stochastic process). \alert{(OPEN PROBLEM)!}
% \end{itemize}
% \end{block}
% \end{frame}

% \begin{frame}\frametitle{Why do I think this is a hard problem?}
% \begin{block}{}
% \begin{itemize}
% \item If $z$ is \alert{not a static decision} variable, but \alert{time-dependent}, then other than $z(0)$, the controls need to be adapted to the underlying stochastic process $\xi_t$.
% \item As more information is revealed in time, $z(t)$ will be dependent on the realization of stochasticity in the time $0 < s < t$.
% \item This usually leads to \textbf{dynamic programming} problems, which even in finite dimensions suffer from the \alert{curse of dimensionality}.
% \item Now, $z$ depends on \textbf{time} $t$, \textbf{uncertainty} $\omega$, \textbf{space} $x \in D$ or $s \in \Gamma$. This only deepens the curse.
% \item \textbf{Open loop} and \textbf{multistage} perspectives are also possible, as has been done in stochastic programming for decades, but what does it mean in this context?
% \end{itemize}
% \end{block}\pause

% \begin{example}[A thought experiment]
% \begin{itemize}
% \item Suppose $z^{\star}$ should give us an optimal policy for drug administration based on a stochastic tumor growth model.
% \item If $z^{\star}_{t}(\omega) \in L^2(\Gamma)$ is the optimal control, how do I know what to do?
% \item Should I (assuming I could) blindly administer whatever a sample path of this process tells me?
% \end{itemize}
% \end{example}
  ...
  \centering
    \begin{tikzpicture}[scale=2.2]
      \node[inner sep=0pt] (result) at (0.5,0.5)
        {\includegraphics[width=0.72\textwidth]{mean_uncontrolled_velocity_vort}};
